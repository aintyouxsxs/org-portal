    ...
    <?php
    session_start();
    include '../functions.php'; // Make sure this includes the database connection
    global $conn;
...
$events = [];
try {
    // Debugging: Check if the orgId is being passed correctly
    error_log('Fetching events for org_id: ' . $orgId);

    $stmt = $conn->prepare("SELECT title, venue, date, start_time, end_time, description, image_url 
                            FROM events 
                            WHERE org_id = :org_id");
    $stmt->bindParam(':org_id', 219, PDO::PARAM_INT);
    $stmt->execute();
    $events = $stmt->fetchAll(PDO::FETCH_ASSOC);

    // Debugging: Check if events were fetched
    error_log('Number of events fetched: ' . count($events));

    if (count($events) === 0) {
        error_log('No events found for this organization.');
    }
            ...
            <div class="organization">
                <div class="container">
                    <div class="org-container">
                        <img src="../uploads/bg.jpg" alt="dwadwa Logo">
                        <div class="org-info">
                            <h1>dwadwa</h1>
                            <h3>About Us</h3>
                            <p>dwad</p>
                ...
                <div class="container">
                    ...
                    <div class="events-grid">
                        <?php if (!empty($events)): ?>
                            ...
                            <?php endforeach; ?>
                        <?php else: ?>
                            <p>No events available for this organization at the moment.</p>
                        <?php endif; ?>
                    </div>
                </div>
            </div>
